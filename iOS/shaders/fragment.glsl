...
const lowp vec4 fogcolor = vec4(0.6, 0.8, 1.0, 1.0);
const lowp float fogdensity = .00003;
...
void main(void) {
	...
	lowp vec4 color = texture2D(texture, coord2d);
	...
	lowp float z = gl_FragCoord.z / gl_FragCoord.w;
	lowp float fog = clamp(exp(-fogdensity * z * z), 0.2, 1.0);
